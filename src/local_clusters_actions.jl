function create_first_local_cluster(group::local_group,cluster_num=1)
    labels = Array(group.labels)
    sub_labels = Array(group.labels_subcluster)
    ...
    cp.suff_statistics[1][1].N = sum(labels .== cluster_num)
    cpl.suff_statistics[1][1].N = sum(sub_labels[labels .== cluster_num] .== 1)
    cpl.suff_statistics[1][1].N = sum(sub_labels[labels .== cluster_num] .== 2)
    cluster = local_cluster(splittable, group.model_hyperparams.total_dim,
        cp.suff_statistics[1][1].N,
        cpl.suff_statistics[1][1].N,
        cpl.suff_statistics[1][1].N,cluster_num)
...
function check_and_split!(group::local_group, final::Bool)
    ...
    if length(indices) > 0
        ...
        if use_smart_splits && length(group.local_clusters[all_indices[1]].cluster_params.cluster_params.suff_statistics)==1
...
function smart_cluster_init!(group::local_group,cluster_num::Int64)
    N = group.local_clusters[cluster_num].cluster_params.cluster_params.suff_statistics[end][1].N
    XXT = group.local_clusters[cluster_num].cluster_params.cluster_params.suff_statistics[end][1].S / N
    μ = group.local_clusters[cluster_num].cluster_params.cluster_params.suff_statistics[end][1].points_sum / N
    M = XXT - μ*μ'
    # println("vecs:")
    F = eigen(M)
    vecs = F.vectors
